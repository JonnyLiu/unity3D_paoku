...
//		}
		...
		protected virtual Texture OnValue_WallTexture
	{
		...
		{	
			...
			if(WallRunObject.renderer == null && m_CurrentTerrain == null)
				return null;
			...
			// return the texture
			return m_CurrentTerrain == null ? WallRunObject.renderer.material.mainTexture : m_CurrentTerrain.terrainData.splatPrototypes[ terrainTextureID ].texture;
...
//			ParkourArmPrefab.transform.position = new Vector3(hit.point.x, m_ClimbObjectPosition.y, hit.point.z) + transform.InverseTransformDirection(ParkourArmPos);
			...
			ParkourArmPrefab.animation.CrossFade(ParkourArmClimbAnim.name);	// play anim
		}
